...
section .data
	buffer times 256 db 0  ; 256-byte buffer initialized to zeros
	x_coord db 0  ; X-coord for cursor position
	y_coord db 0  ; Y-coord for cursor position
...
keyboard_read:
	...
	call echo_char  ; function to display the character
	jmp keyboard_read
...
prev_line:
	...
	dec byte [y_coord]
	mov byte [x_coord], 79  ; Set the x_coord to the rightmost column
...
clear_buffer:
    ...
    mov byte [x_coord], 0  ; Reset X-coordinate to 0
    jmp keyboard_read

new_line:
    inc byte [y_coord]  ; Increment the Y-coordinate for a new line
    mov byte [x_coord], 0  ; Reset X-coordinate to 0
